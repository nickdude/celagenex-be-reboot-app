...
const ProductPage = () => {
    ...
    const handleBuyNow = async () => {
        ...
        try {
            ...
            const orderResponse = await api.post("/auth/user/order", productData);

            if (orderResponse?.status === 201 && orderResponse.data?.order) {
                const orderDetails = orderResponse.data.order;

                // Navigate to NDPS payment page with order details
                navigate("/ndps-payment", {
                    state: {
                        productData,
                        orderId: orderDetails.orderId,
                        orderStatus: orderDetails.status,
                        orderAmount: orderDetails.totalAmount || orderDetails.amount,
                        orderQuantity: orderDetails.quantity || quantity
                    }
                });
